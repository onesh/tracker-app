...
class Map extends Component {
  ...
  setNickName = () => {
    var nickname = '';
    var card  = this.setNickName.card;
    if (this.modalVisible) {
      return (<View
        style={[{opacity: 0.9}, {position: 'absolute', elevation: 4, top: ( Sizes.screen.width / 1.5 ), height: 150, width: Sizes.screen.width - 40, marginLeft: 20, backgroundColor: 'white', }]}
        key = {'nicknameModal'}
        >

        <Text style={{fontSize: 15}}>  set nickname: </Text>
        <TextInput
        ...
        <View style={{width: '30%'}}>
        <Button
        onPress={() => { this.modalVisible = false; card.nickname = nickname; this.forceUpdate(); }}
        title="ok"
        color="black"
        />
        </View>
  ...
  showCard(card, i) {
    ...
    if (card.id === this.activeCard.id) {
      return (
        ...
        <View style={{width: '20%'}}>
        <Button
        onPress={() => { this.modalVisible = true; this.setNickName.card = card; this.forceUpdate()} }
        title="EDIT"
        color="black"
        />
        </View>
  ...
  render = () => {
    ...
    if (this.hasData) {
      ...
      toRender.push(this.setNickName());
      return (
